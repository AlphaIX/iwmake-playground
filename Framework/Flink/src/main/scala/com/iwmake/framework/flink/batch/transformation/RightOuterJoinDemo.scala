...
object RightOuterJoinDemo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 3 转换操作 rightOuterJoin
    val rightJoinAssigner: JoinFunctionAssigner[(Int, String), (Int, String)] = userDS.rightOuterJoin(cityDS).where(0).equalTo(0)
    // 3.1 使用apply方法解析JoinFunctionAssigner中的数据处理逻辑
    val resDS: DataSet[(Int, String, String)] = rightJoinAssigner.apply((left, right) => {
      if (left == null) {
        // 返回数据

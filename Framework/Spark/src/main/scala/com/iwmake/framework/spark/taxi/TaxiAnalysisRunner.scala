...
object TaxiAnalysisRunner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val taxiRaw: Dataset[Row] = spark.read
      ...
      .csv("0_dataset/input/spark/half_trip.csv")
    ...
    // 可以通过如下方式过滤处理所有出异常的row
    //taxiParsed.filter(e => e.isRight)
    //  .map(e => e.right.get._1).collect().foreach(println(_))
    ...
    // 7.增加行政区
    // 7.1 读取数据集
    val geoJson = Source.fromFile("0_dataset/input/spark/nyc-borough-boundaries-polygon.geojson").mkString
    val featureCollection = FeatureExtraction.parseJson(geoJson)
    // 7.2 排序
    // 后续需要得到每个出租车在哪个行政区，拿到经纬度，遍历features搜索其所在行政区
    // 在搜索过程中，行政区越大，命中几率越高，所以把大的行政区放在前面，更容易命中，减少遍历次数
    val sortedFeatures = featureCollection.features.sortBy(feature => {
      (feature.properties("borough"), - feature.getGeometry().calculateArea2D())
    })
    // 7.3 广播
    ...
    val bd = session.mapPartitions(trips => {
